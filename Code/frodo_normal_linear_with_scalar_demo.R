# Simulation study from Section 5 of manuscript
# (Additive Gaussian covariate structure, linear regression model,
# group-level scalar covariate, as in Croon and van Veldhoven (2007))
...
n = sample(c(10, 40), size = N, prob = c(0.5, 0.5), replace = TRUE)
xi = rnorm(N)
z = rnorm(N)
Y = 0.3 + 0.3*z + .3*xi + rnorm(N, 0, sqrt(0.35))
...
# Run FRODO
frodo_runs = frodo_with_scalar(Y, X, r, K, z, orig_bounds = orig_bounds, scale_bounds = scale_bounds, delta = ifelse(n == 10, 0.05, 0.1),
...
naive_scalar_stan_runs = stan('naive_scalar_linmod_with_scalar.stan', data = list(Y = Y_stand, X = sapply(X_stand, mean), N = N, z = z),
                              verbose = TRUE, refresh = 2, iter = 1750, warmup = 750, cores = 4, chains = 4)

# Run hierarchical scalar model
scalar_stan_runs = stan('hierarchical_scalar_normal_linear_with_scalar.stan',
                        data = list(Y = Y_stand, X = unlist(X_stand), N = N, M = length(unlist(X)), n, z = z),
                        verbose = TRUE, refresh = 2, iter = 1750, warmup = 750, cores = 4, chains = 4)

params = extract(stan_runs)
naive_scalar_params = extract(naive_scalar_stan_runs)
scalar_params = extract(scalar_stan_runs)
# save.image('normal_linear_with_scalar')
...
abline(-frodo_runs$data_list$cent_dens %*% diff(0.3*xval^2/2)*K/diff(orig_bounds),
       0.3, lty = 4, lwd = 2)
...
plot(mean_Yhat, Y, main = 'Response values', pch = 16, cex = 0.5,
     xlab = 'Predicted responses', ylab = 'Actual responses', ylim = c(min(lower_CI_Yhat), max(upper_CI_Yhat)))
abline(0, 1, lwd = 1.5)
polygon(c(sort(mean_Yhat), rev(sort(mean_Yhat))), c(lower_CI_Yhat[order(mean_Yhat)], rev(upper_CI_Yhat[order(mean_Yhat)])), col = rgb(0.25,0.25,0.25, 0.25),
        border = NA)
