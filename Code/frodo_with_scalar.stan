...
data{ 
  ...
  vector[N] z; // Group-level covariate
}
...
parameters {
  ...
  real beta_z; // Regression coefficient for z
...
model{
  ...
  Y ~ normal_id_glm(append_col(phi_cent[2:K]', z), alpha, append_row(beta0, beta_z), sigma_y);
...
generated quantities{
  vector[N] fitted_values = append_col(phi_cent[2:K]', z)*append_row(beta0, beta_z) + alpha; // Estimated regression responses
