...
model{
  ...
  int pos = 1;
  for(i in 1:N){
    segment(X, pos, n[i]) ~ exponential(lambda[i]);
    pos += n[i];
  }
  Y ~ normal(alpha + beta*inv(lambda), sigma_Y);

  // Priors
  lambda ~ gamma(10*alpha_lambda, 10*alpha_lambda / mu_lambda);
...
generated quantities{
  vector[N] fitted_values = alpha + beta*inv(lambda);
}
