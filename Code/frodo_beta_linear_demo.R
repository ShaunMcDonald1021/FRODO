# Simulation study from Section 4.4 of manuscript
# (Beta covariate structure, linear regression model)
...
n = rep(15, N) # Individuals per group
xi = seq(0.1, 0.9, length.out = N)
Y = 0.2 + xi + rnorm(N, 0, 0.05)
X = lapply(1:N, function(i) rbeta(n[i], xi[i], 1-xi[i]))
...
# Run naive scalar linear model
naive_scalar_stan_runs = stan('naive_scalar_linmod.stan', data = list(Y = Y_stand, X = sapply(X, mean), N = N),
                              verbose = TRUE, refresh = 2, iter = 1750, warmup = 750, cores = 4, chains = 4)

# Run hierarchical scalar model
scalar_stan_runs = stan('hierarchical_scalar_beta_linear.stan',
                        data = list(Y = Y_stand,
                                    # Standardization necessary to avoid overflow in beta likelihoods
                                    X = standardize(unlist(X), 0, 1, 1e-10, 1-1e-10),
...
plot_stepwise_density(rbind(apply(params$beta,2,mean),apply(params$beta,2,quantile,0.025),
                      ...
                      col = rep(1,3),nlwd = c(2, 0.75, 0.75))
upper_CI = rep(apply(params$beta,2,quantile,0.975)*sd(Y), each = 2)
lower_CI = rep(apply(params$beta,2,quantile,0.025)*sd(Y), each = 2)
polygon(c(xval_plotseq, rev(xval_plotseq)), c(lower_CI, rev(upper_CI)), col = rgb(0.25,0.25,0.25,0.25),
        border = NA)

# True regression function (shifted to have integral 0)
abline(-frodo_runs$data_list$cent_dens %*% diff(1*xval^2/2)*K/diff(orig_bounds),
       1, lty = 4, lwd = 2)

# Regression function from naive scalar model (shifted to have integral 0)
abline(-frodo_runs$data_list$cent_dens %*% (sd(Y)*mean(naive_scalar_params$beta)*diff(xval^2)*K/(2*diff(orig_bounds))),
       mean(naive_scalar_params$beta)*sd(Y), col = 'red', lwd = 1.75)

# Regression function from hierarchical scalar model (shifted to have integral 0)
abline(-frodo_runs$data_list$cent_dens %*% (sd(Y)*mean(scalar_params$beta)*diff(xval^2)*K/(2*diff(orig_bounds))),
       mean(scalar_params$beta)*sd(Y), col = 'blue', lwd = 1.75)
...
polygon(c(xval_plotseq, rev(xval_plotseq)),
        ...
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dbeta(evalseq, xi[min_xi], 1-xi[min_xi]), col = 'red', lwd = 2)
...
plot_stepwise_density(rbind(apply(params$phi[,,mean_xi],2,mean),apply(params$phi[,,mean_xi],2,quantile,0.025),
                      ...
                      ylab = '', lty = c(1, 3, 3), col = rep(1,3), lwd = c(2, 0.75, 0.75), ylim = c(0,11))
polygon(c(xval_plotseq, rev(xval_plotseq)),
        rep(c(apply(params$phi[,,mean_xi],2,quantile,0.025),
              rev(apply(params$phi[,,mean_xi],2,quantile,0.975))),each=2)*K/diff(orig_bounds), 
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dbeta(evalseq, xi[mean_xi], 1-xi[mean_xi]), col = 'red', lwd = 2)
...
plot_stepwise_density(rbind(apply(params$phi[,,max_xi],2,mean),apply(params$phi[,,max_xi],2,quantile,0.025),
                      ...
                      lty = c(1, 3, 3), col = rep(1,3), lwd = c(2, 0.75, 0.75), ylim = c(0,11))
polygon(c(xval_plotseq, rev(xval_plotseq)),
        rep(c(apply(params$phi[,,max_xi],2,quantile,0.025),
              rev(apply(params$phi[,,max_xi],2,quantile,0.975))),each=2)*K/diff(orig_bounds), 
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dbeta(evalseq, xi[max_xi], 1-xi[max_xi]), col = 'red', lwd = 2)
