# Simulation study from Section 4.3 of manuscript
# (Exponential covariate structure, linear regression model)
...
n = rep(50, N) # Individuals per group
lambda = rgamma(N, 10, 10)
Y = 0.1 - 0.9/lambda + rnorm(N, 0, 0.1)
X = lapply(1:N, function(i) rexp(n[i], lambda[i]))
...
# Scale X's, but don't center them here. We need them positive since we assume exponential densities
X_stand_pos = lapply(X, function(x) x/sd(unlist(X)))
scalar_stan_runs = stan('hierarchical_scalar_exp_linear.stan',
                        data = list(Y = Y_stand, X = unlist(X_stand_pos), N = N, M = length(unlist(X)), n = n),
                        verbose = TRUE, refresh = 2, iter = 1750, warmup = 750, cores = 4, chains = 4)

params = extract(stan_runs)
scalar_params = extract(scalar_stan_runs)
naive_scalar_params = extract(naive_scalar_stan_runs)
# save.image('exp_linear')

# Plot regression function (on original scale)
xval = seq(orig_bounds[1], orig_bounds[2], length.out = K+1)
evalseq = seq(orig_bounds[1], orig_bounds[2], length.out = K+1)
...
abline(frodo_runs$data_list$cent_dens %*% (0.9*diff(xval^2)/2)*K/diff(orig_bounds), -0.9, lty = 4, lwd = 2)
...
legend('bottomleft', legend = c('FRODO posterior mean', 'FRODO pointwise 95% C.I.', 'True function', 
...
par(mfrow = c(1,3))
min_xi = which.min(1/lambda)
max_xi = which.max(1/lambda)
mean_xi = which.min(abs(1/lambda - mean(1/lambda)))
...
plot_stepwise_density(rbind(apply(params$phi[,,min_xi],2,mean),apply(params$phi[,,min_xi],2,quantile,0.025),
                      ...
                      ylab = expression(italic(f(x))), lty = c(1, 3, 3), col = rep(1,3), wd = c(2, 0.75, 0.75),
                      ylim = c(0, 1.1))
polygon(c(xval_plotseq, rev(xval_plotseq)),
        ...
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dexp(evalseq, 1/xi[min_xi]), col = 'red', lwd = 2)
...
# Group with true mean closest to mean(1/lambda)
plot_stepwise_density(rbind(apply(params$phi[,,mean_xi],2,mean),apply(params$phi[,,mean_xi],2,quantile,0.025),
                      ...
                      main = bquote(italic(f)[.(mean_xi)]), xlab = expression(italic(x)), ylab = '',
                      lty = c(1, 3, 3), col = rep(1,3), lwd = c(2, 0.75, 0.75), ylim = c(0, 1.1))
polygon(c(xval_plotseq, rev(xval_plotseq)),
        rep(c(apply(params$phi[,,mean_xi],2,quantile,0.025),
              rev(apply(params$phi[,,mean_xi],2,quantile,0.975))),each=2)*K/diff(orig_bounds), 
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dexp(evalseq, 1/xi[mean_xi]), col = 'red', lwd = 2)
...
polygon(c(xval_plotseq, rev(xval_plotseq)),
        ...
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dexp(evalseq, 1/xi[max_xi]), col = 'red', lwd = 2)
