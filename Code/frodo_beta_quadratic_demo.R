# Simulation study from Section 4.5 of manuscript
# (Beta covariate structure, quadratic regression model)
...
# Generate data
set.seed(235711)
N = 250 # Number of groups
n = rep(60, N) # Individuals per group
xi = seq(0.1, 2, length.out = N)
Y = 0.7 + (1+1/(2*xi+1)) + rnorm(N, 0, 0.1)
X = lapply(1:N, function(i) rbeta(n[i], xi[i], xi[i]))
...
# Run FRODO
frodo_runs = frodo(Y, X, r, K, orig_bounds = orig_bounds, scale_bounds = scale_bounds, delta = rep(1, N),
                   verbose = TRUE, refresh = 2, iter = 1750, warmup = 750,
                   control = list(max_treedepth = 12, adapt_delta = 0.99), cores=  4, chains = 4)
...
naive_scalar_stan_runs = stan('naive_scalar_linmod.stan',
                              data = list(Y = Y_stand, X = sapply(X, function(x) mean((x-0.5)^2)), N = N),
                              verbose = TRUE, refresh = 2, iter = 1750, warmup = 750, cores = 4, chains = 4)

# Run hierarchical scalar model
scalar_stan_runs = stan('hierarhical_scalar_beta_quadratic.stan',
                        data = list(Y = Y_stand, X = standardize(unlist(X), 0, 1, 1e-10, 1-1e-10), # Necessary to avoid infinite log-densities
                                    N = N, M = length(unlist(X)), n = n),
                        verbose = TRUE, refresh = 2, iter = 1750, warmup = 750, cores = 4, chains = 4)

params = extract(stan_runs)
scalar_params = extract(scalar_stan_runs)
naive_scalar_params = extract(naive_scalar_stan_runs)
# save.image('beta_squared')
...
plot_stepwise_density(rbind(apply(params$beta,2,mean),apply(params$beta,2,quantile,0.025),
                            apply(params$beta,2,quantile,0.975))*sd(Y),xval, main = 'Regression function',
                      xlab = expression(italic(x)), ylab = expression(italic(beta(x))), lty = c(1, 3, 3),
                      col = rep(1,3), lwd = c(2, 0.75, 0.75))
...
lines(evalseq,
      4*(evalseq - 0.5)^2 - 4*frodo_runs$data_list$cent_dens %*% 
        diff(xval^3/3- 0.5*xval^2 + 0.25*xval)*K/diff(orig_bounds), lty = 4, lwd = 2)

# Regression function from naive scalar model (shifted to have integral 0)
lines(evalseq,
      -frodo_runs$data_list$cent_dens %*% (sd(Y)*mean(naive_scalar_params$beta)*
                                             diff(xval^3/3 - 0.5*xval^2 + 0.25*xval)*K/(diff(orig_bounds)))+
        mean(naive_scalar_params$beta)*sd(Y)*(evalseq-0.5)^2, col = 'red', lwd = 1.75)

# Regression function from hierarchical scalar model (shifted to have integral 0)
lines(evalseq,
      -frodo_runs$data_list$cent_dens %*% (sd(Y)*mean(scalar_params$beta)*
                                             diff(xval^3/3 - 0.5*xval^2 + 0.25*xval))*K/diff(orig_bounds) +
        mean(scalar_params$beta)*sd(Y)*(evalseq-0.5)^2, col = 'blue', lwd = 1.75)
...
plot_stepwise_density(rbind(apply(params$phi[,,min_xi],2,mean),apply(params$phi[,,min_xi],2,quantile,0.025),
                      ...
                      main = bquote(italic(f)[.(min_xi)]), xlab = expression(italic(x)),
                      ylab = expression(italic(f(x))), lty = c(1, 3, 3), col = rep(1,3),
                      lwd = c(2, 0.75, 0.75), ylim = c(0,6))
polygon(c(xval_plotseq, rev(xval_plotseq)),
        rep(c(apply(params$phi[,,min_xi],2,quantile,0.025),
              rev(apply(params$phi[,,min_xi],2,quantile,0.975))),each=2)*K/diff(orig_bounds), 
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dbeta(evalseq, xi[min_xi], xi[min_xi]), col = 'red', lwd = 2)
legend('topright', legend = c('FRODO posterior mean', 'FRODO pointwise 95% C.I.', 'True density'), 
...
# Group with true xi closest to mean(xi)
plot_stepwise_density(rbind(apply(params$phi[,,mean_xi],2,mean),apply(params$phi[,,mean_xi],2,quantile,0.025),
                      ...
                      ylab = '', lty = c(1, 3, 3), col = rep(1,3), lwd = c(2, 0.75, 0.75), ylim = c(0,6))
polygon(c(xval_plotseq, rev(xval_plotseq)),
        rep(c(apply(params$phi[,,mean_xi],2,quantile,0.025),
              rev(apply(params$phi[,,mean_xi],2,quantile,0.975))),each=2)*K/diff(orig_bounds), 
        col = rgb(0.25,0.25,0.25, 0.25), border = NA)
lines(evalseq, dbeta(evalseq, xi[mean_xi], xi[mean_xi]), col = 'red', lwd = 2)

# Group with highest true xi
plot_stepwise_density(rbind(apply(params$phi[,,max_xi],2,mean),apply(params$phi[,,max_xi],2,quantile,0.025),
                      ...
                      main = bquote(italic(f)[.(max_xi)]), xlab = expression(italic(x)), ylab = '',
                      lty = c(1, 3, 3), col = rep(1,3), lwd = c(2, 0.75, 0.75), ylim = c(0,6))
polygon(c(xval_plotseq, rev(xval_plotseq)),
        rep(c(apply(params$phi[,,max_xi],2,quantile,0.025),
              rev(apply(params$phi[,,max_xi],2,quantile,0.975))),each=2)*K/diff(orig_bounds), 
        col = rgb(0.25,0.25,0.25, 0.25),border = NA)
lines(evalseq, dbeta(evalseq, xi[max_xi], xi[max_xi]), col = 'red', lwd = 2)
